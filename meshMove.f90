...
module MeshMove
  ...
  real(8), dimension(:), allocatable :: xpos, ypos, xaux, yaux
...
contains
  ...
  subroutine fluidStructure(dtmin, time, SMOOTH_FIX, x1, y1)
    ...
    real*8 :: x1(npoin), y1(npoin),time1
    integer :: SMOOTH_SIM(2,npoin)
    logical :: SMOOTH_FIX(npoin)
    integer, save :: counter = 0
    integer, parameter :: SMOOTH_INTERVAL = 100
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(.not. allocated(xaux)) allocate(xaux(npoin))
    if(.not. allocated(yaux)) allocate(yaux(npoin))
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if(modulo(counter, SMOOTH_INTERVAL) == 0) then
       !$OMP PARALLEL DO PRIVATE(ipoin)
       do ipoin = 1, npoin
          xaux(ipoin) = x(ipoin)
          yaux(ipoin) = y(ipoin)
...
!    if(time.lt.3.d0)then
    !   ALPHA=0.d0; YPOSR=0.d0
 !   else if(time.gt.3.d0.and.time.lt.4.d0)then
  !     time1=time
   !    time=time-3.d0
       DISN(2)=pi/6.d0*(1-dcos(2*pi*time))/2.d0
       DISN(1)=(1-dcos(pi*time))/2.d0
       ALPHA=DISN(2)-ALPHAV; YPOSR=DISN(1) -YPOSRV
       !time=time1
    !else
       !DISN(2)=0.d0!pi/6.d0*(1-dcos(2*pi*time))/2.d0
       !DISN(1)=0.d0!(1-dcos(pi*time))/2.d0
     !  ALPHA=0.d0; YPOSR=0.d0
    !end if
    !ALPHA = DISN(2) - ALPHAV
    !YPOSR = DISN(1) - YPOSRV
...
!!$   !$OMP PARALLEL DO PRIVATE(IPOIN)
!!$   do ipoin = 1, nmove
!!$      X(ilaux(ipoin)) = x(ilaux(ipoin)) + DxPOS(ilaux(ipoin))
!!$      Y(ilaux(ipoin)) = Y(ilaux(ipoin)) + DYPOS(ilaux(ipoin))
!!$   end do
!!$   !$OMP END PARALLEL DO


    if( modulo(counter, SMOOTH_INTERVAL) == 0) then
       call smoothing(X, Y, inpoel, smooth_fix, npoin, nelem, .1d0)
       !call SUAVIZADO_1(X,Y,inpoel,area,nelem,npoin,nmove,ilaux,nfix_move)
       !$OMP PARALLEL DO PRIVATE(ipoin)
       do ipoin = 1, npoin
          X1(ipoin) = X1(ipoin) + (X(ipoin) - Xaux(ipoin))
          Y1(ipoin) = Y1(ipoin) + (Y(ipoin) - Yaux(ipoin))
          W_X(ipoin) = (X(ipoin) - Xaux(ipoin))/dtmin
          W_Y(ipoin) = (Y(ipoin) - Yaux(ipoin))/dtmin
       end do
    end if

  end subroutine fluidStructure
