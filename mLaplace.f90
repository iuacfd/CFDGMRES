module Mlaplace
  ...
  real(8), dimension(:), allocatable :: lap_sparse, lap_diag, mu_field
...
contains 
  subroutine laplace(inpoel, area, dNx, dNy, nelem, npoin)
    ...
    !$OMP PARALLEL DO PRIVATE(ielem,X3,Y3)
    do ielem=1,nelem
       X3 = X(inpoel(:,ielem))
       Y3 = Y(inpoel(:,ielem))
       mu_field(ielem) = 1.d0/(mu(X3,Y3)**2)
    end do
    !$OMP END PARALLEL DO

    !$OMP PARALLEL DO PRIVATE(ipoin, iesup, ielem, a, i, jpoin, j, kpoin, k, q)
    do ipoin = 1, npoin
