...
PROGRAM NSComp2D
  ...
  GAMM = GAMA
  iter_old=0 ;  TIME = 0.d0
  call RESTART(GAMM,x1,y1,time,iter_old)
  time=0.d0
...
!!$  end do
  ...
  ISAL = 0
  ITER = iter_old
  maxiter=maxiter + iter_old
  DTMIN = 0.d0  ; time1=0.d0
  ITERPRINT = 0
...
!Do While (ITER.LT.MAXITER.AND.ISAL.EQ.0)
	...
	U1(:, ipoin) = U(:, ipoin)
     ...
     if (ITERPRINT.EQ.IPRINT.OR.ITER.EQ.MAXITER) THEN
        ...
        write(7,'(I7, 5E14.6)') ITER, TIME, DSQRT(ER(1)/ERR(1)), DSQRT(ER(2)/ERR(2)), DSQRT(ER(3)/ERR(3)), DSQRT(ER(4)/ERR(4))
        ...
        call PRINTFLAVIA(GAMM, RHO, VEL_X, VEL_Y, P, T, E, RMACH, X1, Y1 &
             , npoin, ITER, mu_field)
        ...
        if(FMU.NE.0.d0)THEN
           call FORCE_VISC(NELEM, npoin, nsets &
                ...
                , F_VX, F_VY, RM, xref, yref)
        end if
        WRITE(17,'(7E13.5)')TIME,F_vX(1),F_vY(1),RM(1),yref(1)
...
!CCCC---->              RESTART           <----CCCC
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC     
subroutine RESTART(GAMM,x1,y1,time,iter_old) 
  !use DATOS_ENTRADA
  use InputData
  use MeshData
  use MVARIABGEN
  ...
  use MVARIABLES
  implicit real(8) (A-H, O-Z)
  real(8) GAMM(npoin),x1(npoin),y1(npoin)
  ...
  if (IRESTART.NE.1) THEN
     ...
     do i=1, npoin
        read(1) (U(J, I), J=1, 4), T(I), GAMM(I)!,X(I),Y(I),W_X(I),W_Y(I),x1(i),y1(i)
     end do
     !$OMP PARALLEL DO PRIVATE(IPOIN, VEL2)
     do ipoin = 1, npoin
	RHO(ipoin) = U(1,ipoin)
        VEL_X(ipoin) = U(2,ipoin)/RHO(ipoin)
	VEL_Y(ipoin) = U(3,ipoin)/RHO(ipoin)
	E(ipoin) = U(4,ipoin)/RHO(ipoin)
...
subroutine PRINTFLAVIA(GAMM, RHO, VEL_X, VEL_Y, P, T, E, RMACH, XPOS, YPOS &
  ...
  real(8) XPOS(npoin), YPOS(npoin), U(4, npoin), mu_field(nelem)
  ...
  if(VEL2CHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Velocity" "Load Analysis"',iter,' Vector OnNodes'
     WRITE(2,'(A)') 'Values'
     do INOD=1, npoin
        write(2, '(I8, 3E13.4)') INOD, VEL_X(INOD), VEL_Y(INOD)
     end do
     WRITE(2,'(A/)') 'End Values'  
  end if
  ...
  if(POSCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Position" "Load Analysis"',iter,' Vector OnNodes'
     WRITE(2,'(A)') 'Values'
  ...
  if(RHOCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Density" "Load Analysis"',iter,' Scalar OnNodes'
     WRITE(2,'(A)') 'Values'
     do INOD=1, npoin
  ...
  if(PRESCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Presure" "Load Analysis"',iter,' Scalar OnNodes'
     WRITE(2,'(A)') 'Values'
     do INOD=1, npoin
  ...
  if(TEMPCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Temperature" "Load Analysis"',iter,' Scalar OnNodes'
     WRITE(2,'(A)') 'Values'
     do INOD=1, npoin
  ...
  if(MACHCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Mach Num" "Load Analysis"',iter,' Scalar OnNodes'
     WRITE(2,'(A)') 'Values'
     do INOD=1, npoin
!        VEL=DSQRT(VEL_X(INOD)*VEL_X(INOD)+VEL_Y(INOD)*VEL_Y(INOD))
!        VC=DSQRT(GAMM(INOD)*FR*T(INOD))
        write(2, '(I8, 2F11.2)') INOD, RMACH(INOD)
     end do
     WRITE(2,'(A/)') 'End Values'
  end if
  ...
  if(ENERCHAR.EQ.'.si.')THEN
     WRITE(2,'(A,I7,A/)') 'Result "Internal Energy" "Load Analysis"',iter,' Scalar OnNodes'
     WRITE(2,'(A)') 'Values'   
     do INOD=1, npoin
...
!!$  WRITE(2,'(A,I7,A)') 'Result "Mu" "Load Analysis"',iter,' Scalar OnGaussPoints "Tri1"'
!!$  WRITE(2,'(A)') 'Values'
!!$  
!!$  DO IELEM=1,NELEM
!!$     WRITE(2,'(I7,E13.3)') IELEM, 1.d0/dsqrt(mu_field(ielem))
!!$  END DO
!!$  WRITE(2,'(A)') 'End Values'
...
subroutine FORCE_VISC(NELEM, npoin, nsets &
  ...
  real(8) F_VX(10), F_VY(10),rm(10), xref(10), yref(10)

  F_VX=0.d0 ; F_VY=0.d0 ; RM=0.d0
  ...
  do ISET_NUMB=1, NSET_NUMB
     ...
     do II=1, IELEM_SETS(ISET_NUMB)
        ...
        smu=110.d0
        fmu=0.0002d0!*(temp/t_inf)**1.5d0*(t_inf+smu)/(temp+smu)
        RLY=-(X(NN2)-X(NN1))
        RLX=Y(NN2)-Y(NN1)
        ...
        RM(ISET_NUMB)=RM(ISET_NUMB)+ttx*rmod*(YC-yref(ISET_NUMB))-tty*rmod*(XC-xref(ISET_NUMB))
  ...
  close(1)
  return
end subroutine FORCE_VISC

!CCCC-----------------------------------------------!CCCC
!CCCC  ----> Imprime resultados para RESTART <----  !CCCC
!CCCC-----------------------------------------------!CCCC 
subroutine PRINTREST(ITER, U, T, GAMM, TIME, filename, x1, y1)
  use MeshData
  use MVELOCIDADES
  implicit real(8) (A-H, O-Z)

  real(8) U(4, npoin), T(npoin), GAMM(npoin),x1(npoin),y1(npoin)
  ...
  do i=1, npoin
     write(1)(U(J, i), J=1, 4), T(i), GAMM(i),X(i),Y(i),W_X(i),W_Y(i),x1(i),y1(i)
  end do

  close(1)

  return
end subroutine PRINTREST
...
! Mueve los nodos como un resorte aplicado al centride del elemento
SUBROUTINE SUAVIZADO_1(Xpos,Ypos,inpoel,area,nelem,npoin,nmove,ilaux,nfix_move)
  implicit none

  INTEGER ipas,inod,npoin,nelem,nmove,ilaux(npoin)
  INTEGER inpoel(3,NELEM)
  integer ielem, n1,n2,n3,i, nfix_move,ipoin

  REAL*8 area(NELEM), ar
  REAL*8 XPOS(npoin),YPOS(npoin)
  REAL*8 AUX1(npoin),AUX2(npoin),AUX3(npoin)
  real(8) fact_x, fact_y, fact
  ...
  DO IPAS=1,100
     ...
     DO IELEM=1,NELEM
        N1=inpoel(1,IELEM); N2=inpoel(2,IELEM); N3=inpoel(3,IELEM)
        ar=area(ielem)
        FACT_X=(XPOS(N1)+XPOS(N2)+XPOS(N3))/(3.d0*ar)
        FACT_Y=(YPOS(N1)+YPOS(N2)+YPOS(N3))/(3.d0*ar)
        FACT=1.D0/ar

        DO I=1,3
           N1=inpoel(I,IELEM)
           AUX1(N1)=AUX1(N1)+FACT_X
           AUX2(N1)=AUX2(N1)+FACT_Y
           AUX3(N1)=AUX3(N1)+FACT
        END DO
     END DO

     DO INOD=1,npoin
        AUX1(INOD)=AUX1(INOD)/AUX3(INOD)
        AUX2(INOD)=AUX2(INOD)/AUX3(INOD)
     END DO

     DO I=1,nmove+nfix_move
        n1=ilaux(i)
        AUX1(N1)=xpos(n1)
        AUX2(N1)=ypos(n1)
     END DO
