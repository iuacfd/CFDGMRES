...
contains
  subroutine smoothing(X, Y, inpoel, fixed, npoin0, nelem0, tol_ini, dX, dY)
    ...
    !%%%%%%%%%%%%%%%
    integer :: iter, ipoin, min_idx,i
    real*8 :: d_max, TOL_DIST, tol_ini
    ...
    if(.not. any(smoothable == .true.)) then
 ...
 real*8 function getMu_min(X, Y, ipoin, inpoel)
   ...
   real*8 :: mu1
   !%%%%%%%%%%%%%%%%%%%%
   if(.not.allocated(esup1).or..not.allocated(esup2)) &
